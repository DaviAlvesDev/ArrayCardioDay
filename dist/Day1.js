...
const transportes = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck'];
//Ex#1: Inventores dos anos 1500
const inventoresSec16 = inventores.filter((inventor) => Math.trunc(inventor.nascimento / 100) == 15);
...
const nomeSobrenome = inventores.map((element) => `${element.nome} ${element.sobrenome}`);
...
const invAnoNasc = inventores.sort(function (inventor1, inventor2) {
    if (inventor1.nascimento > inventor2.nascimento)
        return 1;
    if (inventor1.nascimento < inventor2.nascimento)
        return -1;
    return 0;
...
const invAnosViv = inventores.sort((inventor1, inventor2) => {
...
const bouvelardsComDe = bouvelards.filter((bouvelard) => bouvelard.includes('de'));
...
const instTransportes = transportes.reduce(function (totalTransporte, transporte) {
